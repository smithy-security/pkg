...
const ArtifactRolesElemScannedFile ArtifactRolesElem = "scannedFile"
...
type ExternalProperties struct {
	...
	// a GUID.
	Guid *Guid `json:"guid,omitempty" yaml:"guid,omitempty" mapstructure:"guid,omitempty"`
...
// A stable, unique identifier for many entities in the form of a GUID.
type Guid string

// UnmarshalJSON implements json.Unmarshaler.
func (j *Guid) UnmarshalJSON(b []byte) error {
	type Plain Guid
	var plain Plain
	if err := json.Unmarshal(b, &plain); err != nil {
		return err
	}
	if matched, _ := regexp.MatchString("^[0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[1-5][0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}$", string(plain)); !matched {
		return fmt.Errorf("field %s pattern match: must match %s", "^[0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[1-5][0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}$", "")
	}
	*j = Guid(plain)
	return nil
}
...
// associated with a country or region). The casing is recommended but not required
...
func (j *Language) UnmarshalJSON(b []byte) error {
	...
	if matched, _ := regexp.MatchString("^[a-zA-Z]{2}(-[a-zA-Z]{2})?$", string(plain)); !matched {
		return fmt.Errorf("field %s pattern match: must match %s", "^[a-zA-Z]{2}(-[a-zA-Z]{2})?$", "")
	}
	*j = Language(plain)
	return nil
}
...
// Static Analysis Results Format (SARIF) Version 2.2 JSON Schema: a standard
...
const SchemaJsonVersionA22 SchemaJsonVersion = "2.2"
