...
// normalizePath will take a given path and construct a file url pointing to the file relative to the workspacePath
func (s *SarifTransformer) normalizePath(path string) (*ocsf.File, error) {

	// if path is a file url (file://) we need to get the path after 'file://' before adding it back
	path = strings.TrimPrefix(path, "file://")

	// make the path relative to the optional workspace if possible
	cleanedPath := filepath.Clean(path)
	if s.workspacePath != "" && strings.HasPrefix(cleanedPath, s.workspacePath) {
		pth, err := filepath.Rel(s.workspacePath, cleanedPath)
		if err != nil {
			return nil, errors.Errorf("could not get relative path from path %s using prefix %q", cleanedPath, s.workspacePath)
		}
		cleanedPath = pth
	}
	...
	for _, location := range res.Locations {
		...
		if physicalLocation.ArtifactLocation != nil && physicalLocation.ArtifactLocation.Uri != nil {
			...
			if p := s.detectPackageFromPhysicalLocation(*physicalLocation, pkgType); p != nil {
				...
				finalFile, err := s.normalizePath(uri)
				if err != nil {
...
func (s *SarifTransformer) mergeDataSources(
	...
	case ocsffindinginfo.DataSource_TARGET_TYPE_REPOSITORY:
		...
		if s.isSnykURI(*location.PhysicalLocation.ArtifactLocation.Uri) {
			...
			finalPath, err := s.normalizePath(uri)
			if err != nil {
